jQuery(function($){
  ...
  function after_every_load($container){
    ...
    wcpt_util.do_once_on_container(
      ...
      function( $container ){
        $container.on('click.wcpt_sort_by_column_headings', '.wcpt-heading.wcpt-sortable', window.wcpt_column_heading_sort_handler);
      }
    );
    ...
    $container
      .trigger('wcpt_after_every_load')
      .trigger('wcpt_after_ajax');
  ...
  $('body').on('freeze_table__grab_and_scroll__start', function(){
    $('.wcpt-navigation:not(.wcpt-left-sidebar) .wcpt-dropdown.wcpt-open').removeClass('wcpt-open');
  });
  ...
  function nav_modal(e){
    ...
    $('.wcpt-nm-sort-placeholder [data-wcpt-filter="sort_by"]', $nav_modal).addClass('wcpt-open');
...
(function($){
  ...
  function set_background_color(){
    var $this = $(this),
        ...
        background_color =  ($cell_bg && $cell_bg !== 'rgba(0, 0, 0, 0)') ? $cell_bg : $parent_bg;
  ...
  $('body').on('keyup input', '.wcpt-instant-search .wcpt-search-input', function(e){
    ...
    if( ! val ){
      ...
      $rows.filter('.wcpt-row--category-heading').each(function(){
        ...
        if( $rows.filter('[data-wcpt-group-by-category-slug="'+ cat_slug +'"]:visible').length ){
          $this.removeClass('wcpt-row--instant-search-hidden');
        }
      })
...
(function($){
  ...
  function init_instant_sort(){
    ...
    if( sc_attrs.instant_sort ){
      ...
      // attach data
      var sort_data = [];
      $('.wcpt-row:not(.wcpt-child-row):not(.wcpt-row--category-heading)', $this).each(function(){
        var $this = $(this),
      ...
      wcpt_util.do_once_on_container( $this, 'wcpt-instant-sort-init', function( $container ){
        ...
        $container
          .off('click.wcpt_sort_by_column_headings', '.wcpt-heading.wcpt-sortable')
          .removeClass('wcpt-sortable-headings-init');

        // -- -- attach new handlers
        $container.on('click.wcpt_instant_sort', '.wcpt-heading.wcpt-sortable', function(){
          var $this = $(this),
        ...
        $container.on('change.wcpt_instant_sort', '[data-wcpt-filter="sort_by"] input[type="radio"]', function(e){
      ...
      // turn off
      // : remove init html class of module and turn off handlers
  ...
  window.wcpt_instant_sort = function ( params, $wcpt ){
    ...
    // -- group by category
    if( wcpt_util.get_sc_attrs( $wcpt )[ wcpt_util.get_device() + '_group_by_category'] ){
      $($('.wcpt-row:not(.wcpt-row--category-heading)', $wcpt).get().reverse()).each(function(){
        var $this = $(this),
            cat_slug = $this.attr('data-wcpt-group-by-category-slug'),
            $category_heading = $this.siblings('.wcpt-row--category-heading[data-wcpt-group-by-category-slug="'+ cat_slug +'"]');
    ...
    wcpt_util.do_once_on_container(
      ...
      function( $container ){
        const intersectionObserver = new IntersectionObserver(entries => {
          ...
          if( ! $container.hasClass('wcpt-loading wcpt-infinite-scroll-loading-results') ){
...
(function($){
  window.wcpt_util = {
    ...
    assign_even_odd_row_classes: ($table) => {
      var $rows = $('.wcpt-row:not(.wcpt-child-row, .wcpt-row--category-heading):visible', $table),
    ...
    update_url_by_query: ( query ) => {
      if( typeof window.history !== 'undefined' ){
        ...
        // append param to url
        query = search_params_object.toString() ? '?' + search_params_object.toString() : window.location.pathname;
