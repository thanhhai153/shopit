...
$html_class = trim( apply_filters( 'wcpt_container_html_class', 'wcpt wcpt-' . $data['id'] . ' ' . trim( $this->attributes['class'] ) . ' ' . trim( $this->attributes['html_class'] ) ) );

ob_start();
?>
data-wcpt-table-id="<?php echo $data['id'];?>"
data-wcpt-query-string="<?php echo esc_attr( wcpt_get_table_query_string() ); ?>"
