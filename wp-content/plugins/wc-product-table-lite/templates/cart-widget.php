...
if( 
	...
	if ( WC()->cart->display_prices_including_tax() ) {
		$_total_price =  WC()->cart->get_subtotal() + WC()->cart->get_subtotal_tax();
	} else {
		$_total_price = WC()->cart->get_subtotal();
	}
// -- total	
}else{ 
	if ( wc_prices_include_tax() ) {
		$_total_price =  WC()->cart->get_cart_contents_total() + WC()->cart->get_cart_contents_tax();
	} else {
		$_total_price =  WC()->cart->get_cart_contents_total();
	}
}

$total_price = apply_filters(
	'wcpt_cart_total_price', 
	wcpt_price( $_total_price, true )
);
