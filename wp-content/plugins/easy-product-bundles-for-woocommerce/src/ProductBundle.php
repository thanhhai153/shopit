...
class ProductBundle extends \WC_Product {
	...
	public function is_in_stock() {
		...
		foreach ( $items as $item ) {
			...
			$default_product = isset( $item['default_product'] ) && 'false' !== $item['default_product'] && ! empty( $item['product'] ) ? wc_get_product( (int) $item['product'] ) : null;
	...
	public function get_initial_data( $context = 'view' ) {
		$data = array(
			'product'          => [
				...
				'regular_price'        => '' !== $this->get_regular_price( $context ) ? wc_get_price_to_display( $this, [ 'price' => $this->get_regular_price( $context ) ] ) : '',
				'sale_price'           => '' !== $this->get_sale_price( $context ) && $this->is_on_sale( $context ) ? wc_get_price_to_display( $this, [ 'price' => $this->get_sale_price( $context ) ] ) : '',
	...
	public function get_item_default_data( $item ) {
		...
		$data = array(
			'product'              => isset( $item['default_product'] ) && 'false' !== $item['default_product'] && ! empty( $item['product'] ) ? absint( $item['product'] ) : null,
