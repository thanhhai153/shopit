...
class Assets {
	...
	public function load_product_shared_scripts( $product ) {
		...
		wp_localize_script(
			...
			apply_filters( 'asnp_wepb_localize_product_bundles_shared', array(
				...
				'cssSelectorPosition'              => 'before_css_selector' === $settings->get_setting( 'product_bundle_position', 'before_add_to_cart_button' ) ? 'before' : 'after',
	...
	public function add_custom_styles() {
		...
		if ( ! empty( $styles['product_crossed_out_price_color'] ) && '#ababab' !== $styles['product_crossed_out_price_color'] ) {
			$custom_styles .= '.asnp-product-Price del, .asnp-product-Price del bdi, .asnp-post-grid-price del, .asnp-post-grid-price del bdi, .asnp-productList-price del, .asnp-productList-price del bdi, .asnp-productList-price .asnp-productList-regularPrice, .asnp-productList-price .asnp-productList-regularPrice .woocommerce-Price-amount.amount {';
		...
		if ( ! empty( $styles['product_sale_price_color'] ) && '#606060' !== $styles['product_sale_price_color'] ) {
			$custom_styles .= '.asnp-product-Price bdi, .asnp-product-Price ins, .asnp-product-Price ins bdi, .asnp-post-grid-price bdi, .asnp-post-grid-price ins, .asnp-post-grid-price ins bdi, .asnp-productList-price bdi, .asnp-productList-price ins, .asnp-productList-price ins bdi, .asnp-productList-price .asnp-productList-salePrice,.asnp-productList-price .woocommerce-Price-amount.amount, .asnp-productList-price .asnp-productList-salePrice .woocommerce-Price-amount.amount {';
