...
class ProductBundleHooks {
	...
	public function get_price_html( $price, $product ) {
		...
		if ( 'true' === $product->get_include_parent_price() ) {
			$total   = '' !== $product->get_price() ? (float) $product->get_price() : 0;
		...
		if ( 'from_min' === $price_type ) {
			$content = __( 'From', 'asnp-easy-product-bundles' ) . ' ' . wc_price( wc_get_price_to_display( $product, array( 'price' => $min_price ) ) ) . $product->get_price_suffix();
		} elseif ( 'from_total' === $price_type ) {
			if ( $regular > $total ) {
				$content = __( 'From', 'asnp-easy-product-bundles' ) . ' ' . wc_format_sale_price( wc_get_price_to_display( $product, array( 'price' => $regular ) ), wc_get_price_to_display( $product, array( 'price' => $total ) ) ) . $product->get_price_suffix();
			} else {
				$content = __( 'From', 'asnp-easy-product-bundles' ) . ' ' . wc_price( wc_get_price_to_display( $product, array( 'price' => $total ) ) ) . $product->get_price_suffix();
			}
		} else {
			if ( $regular > $total ) {
				$content = wc_format_sale_price( wc_get_price_to_display( $product, array( 'price' => $regular ) ), wc_get_price_to_display( $product, array( 'price' => $total ) ) ) . $product->get_price_suffix();
			} else {
				$content = wc_price( wc_get_price_to_display( $product, array( 'price' => $total ) ) ) . $product->get_price_suffix();
			}
		}
	...
	public function add_to_cart_validation( $passed, $product_id ) {
		...
		$product_quantity = ! empty( $_REQUEST['quantity'] ) ? (int) $_REQUEST['quantity'] : 1;
		try {
	...
	protected function add_bundle_items_to_cart( $cart_item_key, $product_id, $quantity, $bundle_items ) {
		...
		$is_fixed_price = WC()->cart->cart_contents[ $cart_item_key ]['data']->is_fixed_price();

		WC()->cart->cart_contents[ $cart_item_key ]['asnp_wepb_key']            = $cart_item_key;
		WC()->cart->cart_contents[ $cart_item_key ]['asnp_wepb_is_fixed_price'] = $is_fixed_price;
	...
	protected function add_bundle_item_to_cart( array $args ) {
		if ( empty( $args ) ) {
			return;
		}
		...
		$cart_key = WC()->cart->add_to_cart( $product_id, $quantity, $variation_id, $variation, $cart_data );

		// Remove bundle product when can not add bundle item to the cart.
		if ( empty( $cart_key ) ) {
			$this->remove_bundle_from_cart( $args['cart_item_key'] );
			return;
		}

		if ( ! isset( WC()->cart->cart_contents[ $args['cart_item_key'] ][ self::CART_ITEM_ITEMS_KEY ] ) ) {
			WC()->cart->cart_contents[ $args['cart_item_key'] ][ self::CART_ITEM_ITEMS_KEY ] = [];
		}

		// Search in array is required to make sure that the cart_key is not already in the item.
		if ( ! in_array( $cart_key, WC()->cart->cart_contents[ $args['cart_item_key'] ][ self::CART_ITEM_ITEMS_KEY ] ) ) {
			WC()->cart->cart_contents[ $args['cart_item_key'] ][ self::CART_ITEM_ITEMS_KEY ][] = $cart_key;
		}
	}
	...
	protected function cart_item_price_bundle( $price, $cart_item ) {
		...
		if ( isset( $cart_item['asnp_wepb_is_fixed_price'] ) && $cart_item['asnp_wepb_is_fixed_price'] ) {
			...
			if ( 'true' === $cart_item['data']->get_include_parent_price() ) {
				$regular_price = '' !== $cart_item['data']->get_regular_price() ? (float) $cart_item['data']->get_regular_price() : 0;
			}
		...
		if ( 'true' === $cart_item['data']->get_include_parent_price() ) {
			$price         = '' !== $cart_item['data']->get_price() ? (float) $cart_item['data']->get_price() : 0;
			$regular_price = '' !== $cart_item['data']->get_regular_price() ? (float) $cart_item['data']->get_regular_price() : 0;
		}
	...
	public function cart_loaded_from_session() {
		foreach ( WC()->cart->cart_contents as $cart_item_key => $cart_item ) {
			...
			if ( isset( $cart_item['asnp_wepb_order_again'], $cart_item['asnp_wepb_items'] ) ) {
				$this->add_bundle_items_to_cart( $cart_item_key, $cart_item['product_id'], $cart_item['quantity'], $cart_item['asnp_wepb_items'] );
			}
		}
	}

	public function product_bundle_position_hooks() {
		$position = get_plugin()->settings->get_setting( 'product_bundle_position', 'before_add_to_cart_button' );
	...
	protected function remove_bundle_from_cart( $cart_item_key ) {
		if ( ! isset( WC()->cart->cart_contents[ $cart_item_key ] ) ) {
			return;
		}

		if ( ! empty( WC()->cart->cart_contents[ $cart_item_key ][ self::CART_ITEM_ITEMS_KEY ] ) ) {
			foreach ( WC()->cart->cart_contents[ $cart_item_key ][ self::CART_ITEM_ITEMS_KEY ] as $item_key ) {
